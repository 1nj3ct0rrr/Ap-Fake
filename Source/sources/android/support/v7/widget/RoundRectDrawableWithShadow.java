...
public class RoundRectDrawableWithShadow extends Drawable {
    ...
    public void draw(Canvas canvas) {
        ...
        canvas.translate(Common.Density, this.mRawShadowSize / 2.0f);
        drawShadow(canvas);
        canvas.translate(Common.Density, (-this.mRawShadowSize) / 2.0f);
    ...
    private void drawShadow(Canvas canvas) {
        ...
        if (this.mCardBounds.width() - (2.0f * inset) > Common.Density) {
            drawHorizontalEdges = true;
        } else {
            drawHorizontalEdges = false;
        }
        if (this.mCardBounds.height() - (2.0f * inset) > Common.Density) {
        ...
        if (drawHorizontalEdges) {
            canvas.drawRect(Common.Density, edgeShadowTop, this.mCardBounds.width() - (2.0f * inset), -this.mCornerRadius, this.mEdgeShadowPaint);
        }
        ...
        canvas.drawPath(this.mCornerShadowPath, this.mCornerShadowPaint);
        if (drawHorizontalEdges) {
            canvas.drawRect(Common.Density, edgeShadowTop, this.mCardBounds.width() - (2.0f * inset), this.mShadowSize + (-this.mCornerRadius), this.mEdgeShadowPaint);
        }
        ...
        if (drawVerticalEdges) {
            canvas.drawRect(Common.Density, edgeShadowTop, this.mCardBounds.height() - (2.0f * inset), -this.mCornerRadius, this.mEdgeShadowPaint);
        }
    ...
    private void buildShadowCorners() {
        ...
        this.mCornerShadowPath.moveTo(-this.mCornerRadius, Common.Density);
        this.mCornerShadowPath.rLineTo(-this.mShadowSize, Common.Density);
        ...
        this.mCornerShadowPaint.setShader(new RadialGradient((float) Common.Density, (float) Common.Density, this.mCornerRadius + this.mShadowSize, new int[]{this.mShadowStartColor, this.mShadowStartColor, this.mShadowEndColor}, new float[]{0.0f, this.mCornerRadius / (this.mCornerRadius + this.mShadowSize), 1.0f}, Shader.TileMode.CLAMP));
        this.mEdgeShadowPaint.setShader(new LinearGradient((float) Common.Density, (-this.mCornerRadius) + this.mShadowSize, (float) Common.Density, (-this.mCornerRadius) - this.mShadowSize, new int[]{this.mShadowStartColor, this.mShadowStartColor, this.mShadowEndColor}, new float[]{Common.Density, 0.5f, 1.0f}, Shader.TileMode.CLAMP));
        this.mEdgeShadowPaint.setAntiAlias(false);
    }
